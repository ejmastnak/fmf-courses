...
\begin{minipage}[t]{0.90\textwidth}
    ...
    The curriculum shares many similarities to the probably more familiar ETSETB bachelors degree in \textit{Engineering Physics}, so, for orientation, I have included a comparison of the FMF and ETSETB programs in the \hyperref[s:comparison]{Appendix}.
...
\vspace{2ex}
The \hyperref[s:comparison]{Appendix}, beginning on page \pageref{s:comparison}, gives a comparison of the University of Ljubljana's bachelors degree in \textit{Physics} to the ETSETB's bachelors degree in \textit{Engineering Physics}.
...
\newpage
\appendix
\section*{Comparison to ETSETB Engineering Physics} \label{s:comparison}
Following is a comparison of the University of Ljubljana Faculty of Mathematics and Physics (FMF) bachelors degree in \textit{Physics} to the ETSETB bachelors degree in \textit{Engineering Physics}.
The chief difference: the FMF program lasts three years instead of the ETSETB program's four years, as a result of which:
\begin{itemize}

    \item FMF lacks the courses in biophysics and organic chemistry taught at ETSETB.

    \item FMF lacks the advanced and/or specialized elective courses offered in the 7th semester at ETSETB (e.g. \textit{General Relativity}, \textit{Advanced Materials}, \textit{Computer Simulation of Condensed Matter} etc...).

    \item ETSETB's full-semester undergraduate thesis is replaced by FMF's \hyperref[seminar]{Seminar} course.
    (Although the length of the Seminar paper is less than a traditional undergraduate thesis, the Seminar is still a project of appreciable scope.)

\end{itemize}

\vspace{1ex}
\textbf{How the comparison is organized}
\begin{itemize}

    \item The courses in the ETSETB \textit{Engineering Physics} curriculum appear in italics.

    \item The FMF course(s) covering the same material as each ETSETB course appear in blue text linking to the course description earlier in this document;
    when an ETSETB course is not covered at FMF, this is noted with the text ``Not covered at FMF''.

\end{itemize}
\vspace{2ex}

\begin{chapter}{First semester}
\begin{itemize}
    \item \textit{Calculus 1}\correspondsto \hyperref[mathematics_1]{Mathematics I}
    \item \textit{Inorganic Chemistry}\correspondsto \hyperref[chemistry_1]{Chemistry I}
    \item \textit{Linear Algebra and Geometry}\correspondsto \hyperref[mathematics_2]{Mathematics II}
    \item \textit{Numerical and Computational Methods 1}\correspondsto \hyperref[numerical_methods]{Numerical Methods} (with some material covered more thoroughly in \hyperref[mathematical_physics_lab]{Mathematical Physics Lab})
    \item \textit{Physics 1}\correspondsto \hyperref[classical_physics]{Classical Physics}
\end{itemize}  
\end{chapter}

\begin{chapter}{Second semester}
\begin{itemize}

    \item \textit{Biophysics 1}\correspondsto Not covered at FMF
    \item \textit{Calculus 2}\correspondsto A combination of \hyperref[mathematics_1]{Mathematics I}, \hyperref[mathematics_2]{II}, and \hyperref[mathematics_3]{III}
    \item \textit{Mathematical Methods 1}\correspondsto Differential equations are covered \hyperref[mathematics_3]{Mathematics III}, though systems of non-linear ODEs and n-th order ODEs seem to be covered in less scope in \hyperref[mathematics_3]{Mathematics III} than in \textit{Mathematical Methods 1}.
    \item \textit{Organic Chemistry and Biochemistry}\correspondsto Not covered at FMF
    \item \textit{Physics 2}\correspondsto \hyperref[classical_physics]{Classical Physics}

\end{itemize}   
\end{chapter}

\begin{chapter}{Third semester}
\begin{itemize}

    \item \textit{Mathematical Methods 2}\correspondsto \hyperref[mathematics_4]{Mathematics IV}
    \item \textit{Mechanics}\correspondsto \hyperref[classical_mechanics]{Classical Mechanics}
    \item \textit{Probability and Statistics}\correspondsto Covered partly in \hyperref[probability_in_physics]{Probability in Physics} and in the chapter on Statistics in \hyperref[measurement_techniques]{Measurement Techniques}.
    The ETSETB chapters ``Introduction to Stochastic Processes'' and ``The Poisson Process'' are not covered at FMF; I have self-studied them from MIT OpenCourseWare's course \href{https://ocw.mit.edu/courses/electrical-engineering-and-computer-science/6-041sc-probabilistic-systems-analysis-and-applied-probability-fall-2013/}{6.041SC Probabilistic Systems Analysis and Applied Probability}.
    \item \textit{Quantum Physics}\correspondsto \hyperref[modern_physics_1]{Modern Physics I} and \hyperref[modern_physics_2]{II}
    \item \textit{Thermodynamics}\correspondsto Covered initially in \hyperref[classical_physics]{Classical Physics} and more thoroughly in \hyperref[statistical_thermodynamics]{Statistical Thermodynamics}.
    % although the ETSETB chapter ``Absolute zero and the third law of thermodynamics'' is not covered in the FMF curriculum.

\end{itemize}   
\end{chapter}

\begin{chapter}{Fourth semester}
\begin{itemize}

    \item \textit{Circuit Theory}\correspondsto Covered in the scope of \hyperref[electronics_1]{Electronics I}, \hyperref[measurement_techniques]{Measurement Techniques}, and \hyperref[collecting_and_processing_data]{Collecting and Processing Data}.
    \item \textit{Electromagnetism}\correspondsto Covered initially in \hyperref[classical_physics]{Classical Physics} and more thoroughly in \hyperref[electromagnetic_fields]{Electromagnetic Fields} and \hyperref[optics]{Optics}.
    \item \textit{Instrumentation}\correspondsto Covered in the scope of \hyperref[measurement_techniques]{Measurement Techniques}, \hyperref[collecting_and_processing_data]{Collecting and Processing Data}, and \hyperref[physics_laboratory_1]{Physics Laboratory 1}, \hyperref[physics_laboratory_2]{II}, \hyperref[physics_laboratory_3]{III}, \hyperref[physics_laboratory_4]{IV}, and \hyperref[physics_laboratory_5]{V}.
    \item \textit{Numerical and Computational Methods 2}\correspondsto Covered in \hyperref[numerical_methods]{Numerical Methods} and \hyperref[mathematical_physics_lab]{Mathematical Physics Lab}, although the ETSETB chapter ``Approximation Theory'' is covered in less detail in the FMF curriculum.

    \item \textit{Statistical Physics}\correspondsto \hyperref[statistical_thermodynamics]{Statistical Thermodynamics}

\end{itemize}
\end{chapter}

\begin{chapter}{Fifth semester}
\begin{itemize}

    \item \textit{Electromagnetic Waves}\correspondsto \hyperref[electromagnetic_fields]{Electromagnetic Fields} and \hyperref[optics]{Optics}; the ETSETB chapter on ``Transmission Lines'' is covered in \hyperref[electronics_1]{Electronics I}, but in less detail than at ETSETB.
    \item \textit{Physical Electronics}\correspondsto The ETSETB chapters ``Semiconductor Physics'' and ``PN Junction Diode'' are covered in \hyperref[solid_state_physics]{Solid State Physics}, and some applications of the p-n junction diode are covered in \hyperref[measurement_of_ionizing_radiation]{Measurement of Ionizing Radiation}; the ETSETB chapters on ``Field Effect Transistors'' and the ``Bipolar Junction Transistor'' are covered, albeit lightly, in \hyperref[electronics_1]{Electronics I}).
    \item \textit{Projects of Engineering Physics 1}\correspondsto \hyperref[electronics_laboratory]{Electronics Laboratory} and \hyperref[physics_laboratory_1]{Physics Laboratory I}, \hyperref[physics_laboratory_2]{II}, \hyperref[physics_laboratory_3]{III}, \hyperref[physics_laboratory_4]{IV}, and \hyperref[physics_laboratory_5]{V}.

    \item \textit{Signal Theory}\correspondsto The ETSETB chapters ``Signals and System in the Time Domain'', ``Signals and System in the Transfer Domain'', and ``A/D D/A Conversion'' are covered in \hyperref[collecting_and_processing_data]{Collecting and Processing Data} and in \hyperref[electronics_1]{Electronics I} and \hyperref[electronics_2]{II};
    I have also self-studied the Hilbert space geometric formulation of signals and systems from Vetterli, Kova\v{c}evi\'{c}, and Goyal's book \href{https://www.fourierandwavelets.org/FSP_v1.1_2014.pdf}{\textit{Foundations of Signal Processing}}.
    The ETSETB chapters ``Stochastic Processes -- Random Signals'', ``Fundamentals of Estimation Theory'', ``Spectral Estimation'', and ``Optimal Wiener Filtering'' are not covered in the FMF curriculum; I have self-studied the material from a combination of MIT OpenCourseWare's \href{https://ocw.mit.edu/courses/electrical-engineering-and-computer-science/6-041sc-probabilistic-systems-analysis-and-applied-probability-fall-2013/}{6.041SC} and \href{https://www.fourierandwavelets.org/FSP_v1.1_2014.pdf}{\textit{Foundations of Signal Processing}}.

    \item \textit{Solid State}\correspondsto \hyperref[solid_state_physics]{Solid State Physics}

\end{itemize}
\end{chapter}

\begin{chapter}{Sixth semester}
\begin{itemize}
    \item \textit{Biophysics 2}\correspondsto Not covered at FMF
    \item \textit{Control Theory}\correspondsto \hyperref[electronics_1]{Electronics I}, \hyperref[electronics_2]{II}, and \hyperref[measurement_techniques]{Measurement Techniques}, although material is covered in less detail than at ETSETB.
    \item \textit{Photonics}\correspondsto The contents of ``Part I'' are covered in \hyperref[optics]{Optics}.
    ``Part II'' is not formally covered in the FMF curriculum, but applications of photonics appear regularly in the measurement techniques and experiments in \hyperref[physics_laboratory_2]{Physics Laboratory II}, \hyperref[physics_laboratory_3]{III}, \hyperref[physics_laboratory_4]{IV}, and \hyperref[physics_laboratory_5]{V}.

    \item \textit{Projects of Engineering Physics 2}\correspondsto The exact ETSETB projects are not covered in the FMF curriculum, but see the potentially relevant experiments in \hyperref[physics_laboratory_3]{Physics Laboratory III}, \hyperref[physics_laboratory_4]{IV}, and \hyperref[physics_laboratory_5]{V}.

    \item \textit{Quantum Mechanics}\correspondsto \hyperref[quantum_mechanics]{Quantum Mechanics}
\end{itemize}
\end{chapter}

\begin{chapter}{Seventh semester}
\begin{itemize}
    \item \textit{Advanced Materials}\correspondsto Not covered at FMF
    \item \textit{Astrophysics and Cosmology}\correspondsto Not covered at FMF
    \item \textit{Biomedical Photonics}\correspondsto Not covered at FMF
    \item \textit{Computational Biophysics}\correspondsto Not covered at FMF
    \item \textit{Computer Simulation of Condensed Matter}\correspondsto Not covered at FMF
    \item \textit{General Relativity}\correspondsto Not covered at FMF
    \item \textit{Nanotechnology}\correspondsto Not covered at FMF
    \item \textit{Physics of Fluids}\correspondsto \hyperref[mechanics_of_continuous_media]{Mechanics of Continuous Media} covers Chapters 1 and 2 in detail, but Chapters 3-5 only in passing.
    \item \textit{Quantum Optical Technologies}\correspondsto Not covered at FMF
\end{itemize}   
\end{chapter}

\begin{chapter}{Eighth Semester}
    The FMF \hyperref[seminar]{Seminar} course replaces the ETSETB undergraduate thesis.
    You can view my Seminar project at \href{https://ejmastnak.github.io/seminar/seminar.html}{\texttt{www.ejmastnak.github.io/seminar/seminar.html}}
\end{chapter}
